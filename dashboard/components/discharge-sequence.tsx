...
const STEPS: readonly Step[] = [
  {
    clock: "0:00",
    title: "Four roles",
    body: "A client, a worker agent, the underwriter pool and the reinsurer pool — each an on-chain account with its own wallet and role. Identity and reliability come from the AgentRegistry contract.",
  ...
  },
  {
    ...
    body: "The client asks the underwriter for a quote on the worker. RiskEngine returns a premium derived from the worker's on-chain reliability. Raise the reliability and the premium collapses. Nothing here is hardcoded.",
  ...
  },
  {
    ...
    body: "The underwriter accepts the negotiation with its PolicyPool as providerFundAddress. CAP moves the fundAmount to the pool in the same transaction that pays the order. There is no second transfer that can fail.",
  ...
  },
  {
    ...
    body: "Unprompted, the underwriter opens a CAP order against the reinsurer and cedes 50% of the risk. Provider and requester in the same breath — structural composability, not decorative.",
  ...
  },
  {
    ...
    body: "The client hires the worker. The worker fails to deliver. The client calls rejectOrder and the CAP order enters its terminal rejected state. That state, not an oracle we wrote, is the definition of failure.",
  ...
  },
  {
    ...
    body: "The watcher sees order_rejected, signs an EIP-712 attestation, and submits it to ClaimsAdjudicator. The contract verifies the signature, checks the policy is live, and pays the client in full — no human approval.",
  ...
  },
  {
    clock: "3:45",
    title: "The cascade",
    body: "ClaimsAdjudicator then bills the reinsurer pool for its quota share. Two layers, one line of settlement, zero human approvals.",
  ...
  },
  {
    ...
    body: "The worker's reliability index falls. The next quote on it is more expensive for everyone, not just this client. Reputation stops being a badge and starts being a price.",
    footnote: "reliability ↓ · premium ↑",
    tone: "neutral",
  },
];
